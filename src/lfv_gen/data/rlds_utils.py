...
def make_d4rl_transition_dataset(dataset):
    dataset = dataset.map(skip_truncated_last_step, num_parallel_calls=tf.data.AUTOTUNE)

    transitions = episodes_to_transitions_dataset(
        dataset,
        cycle_length=16,
        block_length=16,
